...
class TestEtcd3Gateway(base.TestCase):
    def test_client_version_discovery(self):
        ...
        with mock.patch.object(client, "session") as mock_session:
            ...
            self.assertEqual("http://localhost:2379", client.base_url)
            self.assertEqual(
                "http://localhost:2379/v3/lease/grant",
    ...
    def test_client_version_discovery_v3beta(self):
        ...
        with mock.patch.object(client, "session") as mock_session:
            ...
            self.assertEqual("http://localhost:2379", client.base_url)
            self.assertEqual(
                "http://localhost:2379/v3beta/lease/grant",
                client.get_url("/lease/grant"),
            )
    ...
    def test_client_version_discovery_v3alpha(self):
        ...
        with mock.patch.object(client, "session") as mock_session:
            ...
            self.assertEqual("http://localhost:2379", client.base_url)
            self.assertEqual(
                "http://localhost:2379/v3alpha/lease/grant",
                client.get_url("/lease/grant"),
            )
    ...
    def test_client_version_discovery_fail(self):
        ...
        with mock.patch.object(client, "session") as mock_session:
            ...
            mock_session.get.return_value = mock_response
            self.assertRaises(
                exc.Etcd3Exception, client.get_url, "/lease/grant"
            )
    ...
    def test_client_ipv4(self):
        ...
        self.assertEqual("http://127.0.0.1:2379", client.base_url)
        self.assertEqual(
            "http://127.0.0.1:2379/v3/lease/grant",
            client.get_url("/lease/grant"),
        )

    def test_client_ipv6(self):
        client = Etcd3Client(host="::1", api_path='/v3/')
        self.assertEqual("http://[::1]:2379", client.base_url)
        self.assertEqual(
            "http://[::1]:2379/v3/lease/grant", client.get_url("/lease/grant")
        )
    ...
    def test_client_status(self):
        ...
        with mock.patch.object(client, "session") as mock_session:
            ...
            mock_session.post.assert_has_calls(
                [
                    mock.call(
                        'http://127.0.0.1:2379/v3/maintenance/status',
                        timeout=None,
                        json={},
                    )
                ]
